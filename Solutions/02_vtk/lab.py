...
def zero_cut(m):
    if m < 0:
        return 0
    return m
np_zero_cut = np.vectorize(zero_cut)
...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        self.velocity[2] = -np_zero_cut((np.sqrt(np.power(cx - self.nodes[0], 2) + np.power(cy - self.nodes[1], 2))) - cx/5.65) * \
                            (cz > 3/5*self.nodes[2])
    ...
    def snapshot(self, snap_number):
        ...
        writer.SetFileName("Solutions/trash/bacteriophage-step-" + str(snap_number) + ".vtu")
        writer.Write()

# построит сетку
def mesh_stl(relpath):
    # Восстановим геометрию
    gmsh.model.mesh.classifySurfaces(0.9, True, True)
...
try:
    ...
    gmsh.merge(os.path.join(path, '../trash/01_task_mesh.msh'))
except:
    print("Could not load mesh file: bye!")
    gmsh.finalize()
    exit(-1)
...
for i in range(1, shots):
    mesh.move(0.05)
    mesh.snapshot(i)
    print(f"Info    : [{int(i/shots*100)}%] Rendering")
